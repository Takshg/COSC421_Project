...
def create_profile(nlp, matcher, text):
    """
    Create a skill profile for a given text (e.g., resume or job description).

    :param nlp: Loaded spaCy NLP model
    :param matcher: PhraseMatcher initialized with skill data
    :param text: Text data to analyze (e.g., job description)
    :return: DataFrame containing matched skills and their counts
    """
    ...
    # Extract skills and sub-skills from matches
    matched_data = []
    for match_id, start, end in matches:
        rule_id = nlp.vocab.strings[match_id]  # Skill name
        span = doc[start:end]  # Sub-skill text
        matched_data.append((rule_id, span.text))

    # Count occurrences of each skill-sub-skill pair
    data = []
...
if __name__ == '__main__':
    ...
    # Process each job description and create skill profiles
